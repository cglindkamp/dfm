...
START_TEST(test_commandline_history_white_space)
{
	struct commandline cmdline;
	...
	assert_oom_cleanup(commandline_start(&cmdline, L':') != ENOMEM, commandline_destroy(&cmdline));
	ck_assert_int_eq(commandline_history_add(&cmdline, L""), EINVAL);
	ck_assert_int_eq(commandline_history_add(&cmdline, L"	 "), EINVAL);
	assert_oom_cleanup(commandline_history_add(&cmdline, L"foo") != ENOMEM, commandline_destroy(&cmdline));
...
Suite *commandline_suite(void)
{
	...
	tcase_add_test(tcase, test_commandline_history_white_space);
